    ...
    float *const result_to)
{
  ...
  const int *const relevant_face_edge_neighbor_counts = face_edge_neighbor_counts + relevant_face_vertex_offset;
  ...
  const float to_surface_distance = to_surface_difference[0] * relevant_face_normal[0] + to_surface_difference[1] * relevant_face_normal[1] + to_surface_difference[2] * relevant_face_normal[2];
  ...
  if (to_surface_distance < 0.0f)
  {
    ...
    const float from_surface_distance = from_surface_difference[0] * relevant_face_normal[0] + from_surface_difference[1] * relevant_face_normal[1] + from_surface_difference[2] * relevant_face_normal[2];

    if (from_surface_distance >= 0.0f)
    {
      const float adjusted_from_surface_distance = from_surface_distance - tolerance;
      const float adjusted_to_surface_distance = to_surface_distance - tolerance;

      best_along = adjusted_from_surface_distance / (adjusted_from_surface_distance - adjusted_to_surface_distance);
  ...
  for (int vertex_index = 0; vertex_index < relevant_face_vertex_count; vertex_index++)
  {
    ...
    const float to_edge_distance = to_edge_difference[0] * relevant_edge_exit_normal[0] + to_edge_difference[1] * relevant_edge_exit_normal[1] + to_edge_difference[2] * relevant_edge_exit_normal[2];
    ...
    if (to_edge_distance > 0.0f)
    {
      ...
      const float from_edge_distance = from_edge_difference[0] * relevant_edge_exit_normal[0] + from_edge_difference[1] * relevant_edge_exit_normal[1] + from_edge_difference[2] * relevant_edge_exit_normal[2];

      if (from_edge_distance <= 0.0f)
      {
        if (relevant_face_edge_neighbor_counts[vertex_index] == 0)
        {
          const float adjusted_from_edge_distance = from_edge_distance + tolerance;
          const float adjusted_to_edge_distance = to_edge_distance + tolerance;

          const float along = adjusted_from_edge_distance / (adjusted_from_edge_distance - adjusted_to_edge_distance);
        ...
        else
        {
          ...
          if (along <= best_along)
          {
            const float secondary_adjusted_from_edge_distance = from_edge_distance - tolerance;
            const float secondary_adjusted_to_edge_distance = to_edge_distance - tolerance;

            best_along = along;
            best_escape = secondary_adjusted_from_edge_distance / (secondary_adjusted_from_edge_distance - secondary_adjusted_to_edge_distance);
            output = vertex_index;
          }
        }
      }
  ...
  {
    const float forward = best_along > 1.0f ? 1.0f : (best_along < 0.0f ? 0.0f : best_along);
  ...
  default:
  {
    const float forward = best_escape > 1.0f ? 1.0f : (best_escape < 0.0f ? 0.0f : best_escape);
    const float inverse = 1.0f - forward;
